# Spec: 수식/다이어그램 렌더링 (KaTeX · Mermaid)
...
- **DBML** — 후보였던 `@softwaretechnik/dbml-renderer@1.0.31`이 `@aduh95/viz.js/sync`(Node worker_threads)를 require하여 브라우저에서 실행 불가. v1.0 MVP에서 제외. DB 스키마는 Mermaid `erDiagram`으로 대체. 별도 슬라이스에서 브라우저 호환 렌더러 탐색 예정.
- 수식 에디터/미리보기 — v1.0 범위 밖

## 아키텍처

```
view/+page.svelte — note 변경:
  article = <article class="markdown-body">{@html note.html}</article>
  await renderMarkdownPipeline(article)
...
2. Prism은 `language-mermaid`를 건너뛰므로(1.3a 계약), Mermaid가 원본을 읽을 수 있다.
3. Mermaid는 `<pre><code>`를 **`<div class="diagram">`으로 치환**하기 때문에, 토큰 하이라이팅이 끝난 뒤 실행해야 한다.

각 단계는 독립적으로 try/catch. 한 단계 실패가 다음 단계를 막지 않는다.

### Mermaid 사전 검증

`mermaid.render()`는 실패 시 body에 임시 에러 SVG 컨테이너를 남길 수 있다(11.x 관찰). 따라서:
1. `mermaid.parse(source, { suppressErrors: true })`로 **먼저 문법 검증**
2. `false` 반환이면 `render()` 호출하지 않고 바로 fallback 경로
3. 그래도 `render()`가 실패하면 `document.getElementById("d"+id)`/`document.getElementById(id)`를 제거해 잔여물을 정리
