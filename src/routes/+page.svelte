...
<div class="p-6 max-w-5xl">
  <div class="mb-6">
    ...
    {#if vaultName}
      <div class="text-sm text-muted mt-1">📓 {vaultName}</div>
    {/if}
  </div>
  ...
  {:else}
    ...
    <div class="grid grid-cols-4 gap-3 mb-6">
      <div class="bg-surface-1 rounded-lg p-4 border border-border">
        ...
        <div class="text-xs text-muted mt-1">Total Notes</div>
      </div>
      <div class="bg-surface-1 rounded-lg p-4 border border-border">
        <div class="text-2xl font-bold">{stats.total_links}</div>
        <div class="text-xs text-muted mt-1">Links</div>
      </div>
      <div class="bg-surface-1 rounded-lg p-4 border border-border">
        <div class="text-2xl font-bold">{stats.total_tags}</div>
        <div class="text-xs text-muted mt-1">Tags</div>
      </div>
      <div class="bg-surface-1 rounded-lg p-4 border border-border">
        <div class="text-2xl font-bold {stats.orphan_notes > 0 ? 'text-warning' : ''}">{stats.orphan_notes}</div>
        <div class="text-xs text-muted mt-1">Orphan Notes</div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-4">
      <!-- By Type -->
      <div class="bg-surface-1 rounded-lg p-4 border border-border">
        <h3 class="text-sm font-medium mb-3 text-muted">By Type</h3>
        ...
        {#if Object.keys(stats.by_type).length === 0}
          <p class="text-xs text-muted">No typed notes</p>
        {/if}
      </div>

      <!-- By Status -->
      <div class="bg-surface-1 rounded-lg p-4 border border-border">
        <h3 class="text-sm font-medium mb-3 text-muted">By Status</h3>
        ...
        {#if Object.keys(stats.by_status).length === 0}
          <p class="text-xs text-muted">No status set</p>
        {/if}
      </div>

      <!-- Tags Heatmap -->
      <div class="bg-surface-1 rounded-lg p-4 border border-border overflow-hidden">
        <h3 class="text-sm font-medium mb-3 text-muted">Tags</h3>
        {#if tags.length > 0}
      ...
      >
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="text-sm font-medium text-white">Claude 도구</div>
          ...
          <span class="text-xs text-muted">자세히 →</span>
        </div>
      </a>
    {/if}
    ...
    <!-- Recent Notes -->
    <div class="mt-6">
      <h3 class="text-sm font-medium mb-3 text-muted">Recent Notes</h3>
        ...
        {#each recentNotes as note}
          ...
          >
            <div class="flex items-center gap-3 min-w-0">
              ...
              {#if note.frontmatter}
                <span class="text-xs px-1.5 py-0.5 rounded bg-surface-3 text-muted shrink-0">
                  {typeLabel(note.frontmatter.note_type)}
                </span>
              {/if}
            </div>
            <span class="text-xs text-muted shrink-0 ml-3">{formatDate(note.modified_at)}</span>
          </a>
        {/each}
