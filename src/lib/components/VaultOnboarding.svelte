  ...
  <div class="w-full max-w-xl">
      ...
      <div class="grid gap-4">
        ...
        >
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              <h2 class="text-base font-semibold text-fg mb-1">새 볼트 생성</h2>
              <p class="text-sm text-fg-muted">
        ...
        >
          <div class="flex items-start gap-4">
            <div class="text-3xl">📂</div>
            <div class="flex-1">
              <h2 class="text-base font-semibold text-fg mb-1">기존 볼트 연결</h2>
              <p class="text-sm text-fg-muted">
    ...
    {:else}
      <div class="bg-surface-1 border border-border rounded-xl p-5">
        ...
        <div class="font-mono text-sm text-fg break-all mb-4">{parentDir}</div>

        <label for="vault-name" class="text-xs text-fg-muted block mb-1">볼트 이름</label>
        <input
          id="vault-name"
          type="text"
        ...
        {#if targetPath && nameIsValid}
          <div class="mt-3 text-xs text-fg-muted">
            생성 위치:
            <span class="font-mono text-fg">{targetPath}</span>
          </div>
        {/if}

        <div class="flex justify-end gap-2 mt-5">
          <button
            class="text-sm px-4 py-2 rounded border border-border text-fg-muted
                   hover:text-fg hover:border-accent transition-colors
                   disabled:opacity-50"
